...
data(logMISCdataValidate)
round(logMISCdataValidate[1:5,c(1:2, 20:21,33:34,50:51 )], digits=2) 
```
...
```{r, echo=TRUE}
geneList <- matrix(unlist(strsplit(x=rownames(logMISCdataValidate), split ="_")),
                   ncol=2, byrow=TRUE)[,1]
head(geneList)
```
...
```{r, echo=TRUE}
grpIndUse <- matrix(unlist(strsplit(x=names(logMISCdataValidate), split="[.]")),
                    ncol=2, byrow=TRUE)[,1]
table(grpIndUse)
```
...
pred.Set2 <- predictSignature(genesUse=geneSet2, geneNames=geneList,
                           grpInd=grpInd, log2out=logMISCdataValidate, cv=TRUE)
```
...
pred.Set.Nygaard <- predictSignature(genesUse=geneSetNygaard, 
                        ...
                        log2out=logMISCdataValidate, cv=TRUE)
